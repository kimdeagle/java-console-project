...
import com.garden.used.admin.site.InputSite;
...
//			System.out.println();
	...
	public static void DeltEvent(String PostNum) throws IOException { // 공지사항 지우기
		...
		try {
			...
			for (int i = 0; i < Post.size(); i++) { // null처리하는 삭제법
				String[] each_Info = Post.get(i).split("■");
				if (each_Info[0].equals(PostNum)) {
					i++;
				}
			...
			int[] number = new int[RPost.size()];
			for (int i = 0; i < Post.size(); i++) {
				String[] each_Info = RPost.get(i).split("■");
				if (each_Info[0].equals(PostNum)) {
					bw.write(String.format("%08d", number[i] = i + 1));
					bw.write("■");
					bw.write(String.format("%s", each_Info[1]));
					bw.write("■");
					bw.write(String.format("%s", each_Info[2]));
					bw.write("■");
					bw.write(String.format("%s", each_Info[3]));
					bw.write("■");
					bw.write(String.format("%s", each_Info[4]));
					bw.write("■");
					bw.write(String.format("%s", each_Info[5]));
					bw.newLine();
